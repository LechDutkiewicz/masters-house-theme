...
function bon_meta_box_post_display_seo( $object, $box ) {
	...
	<?php /* 
	 <p>
		...
		<label for="bon-meta-index"><?php _e( 'Index this page:', 'bon' ); ?></label>
		<br />
		Yes: <input type="radio" name="bon-meta-index" value="true" <?php
		if ( esc_attr( get_post_meta( $object->ID, 'Index', true ) ) == 'true' || ( esc_attr( get_post_meta( $object->ID, 'Index', true ) ) == '' ) ) {
			echo 'checked';
		}
		?>/>
		No: <input type="radio" name="bon-meta-index" value="false" <?php
				   if ( esc_attr( get_post_meta( $object->ID, 'Index', true ) ) == 'false' ) {
					   echo 'checked';
				   }
				   ?>/>
	</p>
