...
function get_the_image( $args = array() ) {
	...
	/* Or, if $echo is set to false, return the formatted image. */ elseif ( false === $echo ) {
...
function get_the_image_by_post_thumbnail( $args = array() ) {
	...
	return array( 'src' => $image[0], 'post_thumbnail_id' => $post_thumbnail_id, 'alt' => $alt, 'width' => $image[1], 'height' => $image[2] );
}
...
function get_the_image_by_attachment( $args = array() ) {
	...
	/* If the post is not an attachment, check if it has any image attachments. */ elseif ( 'attachment' !== $post_type ) {
...
function get_the_image_format( $args = array(), $image = false ) {
	...
	if ( is_array( $image_class ) ) {
		foreach ( $image_class as $single_class ) {
			$classes[] = sanitize_html_class( $single_class );
		}
	} else {
