...
function shandora_render_builder_element_listing( $value ) {
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) : the_post();
			...
			$term_meta = wp_get_post_terms( get_the_ID(), 'property-type' );
			$li_class = $term_meta[0]->slug;
...
function shandora_override_contact_form_widget( $output, $email, $color = '' ) {
	...
	$o .= '<button type="submit" class="submit flat button ' . $color . ' radius" name="submit"'.get_ga_event('Newsletter', 'Subscribe', 'Sidebar').'>' . __( 'Subscribe', 'bon' ) . '</button>';
